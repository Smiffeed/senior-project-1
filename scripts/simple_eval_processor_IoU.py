#!/usr/bin/env python3
"""
Simple eval_percent processor with clean output filtering
"""
...
def process_single_file(csv_file, model_path, threshold, output_base_dir):
    ...
    cmd = [
        python_exe,
        "scripts/evaluate_advanced_models_IoU.py",
...
"""
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            with open(note_file, 'w', encoding='utf-8') as f:
                f.write(header)
                f.write(f"=== Evaluation failed with return code: {result.returncode} ===\n")
        ...
        print(f"Evaluation interrupted by user")
        with open(note_file, 'w', encoding='utf-8') as f:
            f.write(header)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Simple eval_percent processor with clean output")
    ...
    print(f"Simple eval_percent processor (clean output)")
