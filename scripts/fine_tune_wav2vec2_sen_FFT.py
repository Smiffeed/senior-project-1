...
def apply_fft_processing(audio_np, sr=16000):
    """Apply FFT-based processing for noise reduction and enhancement"""
    # Compute FFT
    ...
    noise_threshold = np.mean(magnitude) * 0.1
    fft[magnitude < noise_threshold] = 0
    
    # Enhance speech frequencies (300Hz - 3400Hz)
    speech_mask = (np.abs(freqs) >= 300) & (np.abs(freqs) <= 3400)
    fft[speech_mask] *= 1.2  # Boost speech frequencies
    
    # Inverse FFT
    processed_audio = np.real(np.fft.ifft(fft))
    
    return processed_audio

def preprocess_audio(file_path, start_time, end_time, max_length=16000):
    """Load and preprocess audio segment with FFT-based enhancement"""
    # Get the sample rate
    ...
    # Apply FFT-based processing
    audio_np = apply_fft_processing(audio_np, sr)
