...
def plot_confusion_matrix(true_labels, pred_labels, labels):
    ...
    try:
        ...
        plt.savefig(f'{plots_dir}/confusion_matrix_advanced.png', 
                    bbox_inches='tight', 
                    dpi=300)
        plt.close()
...
def main():
    ...
    try:
        ...
        plt.tight_layout()
        plots_dir = get_plots_directory()
        plt.savefig(f'{plots_dir}/det_curve_binary_profanity.png', dpi=300)
        plt.close()
        print(f"✅ DET curve saved to {plots_dir}/det_curve_binary_profanity.png")
    except Exception as e:
    ...
    if len(word_predictions) > 0 and len(true_words) > 0:
        ...
        if len(word_level_true) > 0:
            ...
            print(f"✅ Word-level confusion matrix saved to {plots_dir}/confusion_matrix_advanced.png")
        else:
    ...
    if len(profanity_results_filtered) > 0:
        ...
        def plot_window_confusion_matrix(true_labels, pred_labels, labels):
            ...
            try:
                ...
                plt.savefig(f'{plots_dir}/confusion_matrix_window_level.png', 
                            bbox_inches='tight', 
                            dpi=300)
                plt.close()
        ...
        print(f"✅ Window-level confusion matrix saved to {plots_dir}/confusion_matrix_window_level.png")
