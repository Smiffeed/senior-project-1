#!/usr/bin/env python3
"""
Smart eval_percent batch processor - processes smallest datasets first
"""
...
def main():
    parser = argparse.ArgumentParser(description="Smart eval_percent batch processor")
    ...
    print(f"Smart eval_percent batch processor")
    ...
    for config in all_configs:
        ...
        if args.skip_large and config['dataset_size'] > 100000:
            print(f"Skipping large dataset: {config['relative_path']} ({config['dataset_size']:,} windows)")
            continue
    ...
    print(f"\nPROCESSING PLAN (ordered by size):")
    ...
    estimated_hours = total_estimated_windows / 1000 / 60  # Very rough estimate
    print(f"   Estimated Total Time: {estimated_hours:.1f}-{estimated_hours*2:.1f} hours")
    ...
    for i, config in enumerate(filtered_configs, 1):
        ...
        print(f"Session Stats: Success {successful} | Failed {failed} | {total_time/60:.1f}min total")
