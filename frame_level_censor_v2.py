...
class SimpleAudioPreprocessor:
    """Simple audio preprocessor matching evaluation script"""
...
class AdvancedFrameLevelCensorV2:
    """
    Advanced frame-level censoring system V2 - No gap tolerance merging
    Uses word-level detection with ensemble models for best accuracy
    Merges any profanity words that overlap or are directly adjacent (no gap)
    """
    ...
    def _load_ensemble_models(self, num_folds=5):
        ...
        # First, check if this is a standard HuggingFace model directory
        hf_files = ['config.json', 'model.safetensors', 'preprocessor_config.json']
        model_path = Path(self.model_dir)
        
        # Check if all HuggingFace files exist
    ...
    def _merge_overlapping_windows(self, windows: List[Tuple[float, float, str, float]], max_gap: float = 0.0) -> List[Tuple[float, float, str, float]]:
        """
        V2: Merge overlapping profanity windows with NO gap tolerance
        - Merges any profanity words (regardless of label) 
        - Only if overlapping or directly adjacent (no gap)
        - Keeps label with higher confidence
        """
        if not windows:
            return []
        ...
        for next_start, next_end, next_label, next_conf in windows[1:]:
            # V2: Merge any profanity if overlapping or directly adjacent (no gap)
            if next_start <= current_end:  # No gap tolerance
                ...
                print(f"  🔗 V2 Merged: {current_label} at {current_start:.2f}-{current_end:.2f}s (conf: {current_conf:.3f})")
            else:
        ...
        print(f"📋 V2 Merged: {len(windows)} windows → {len(merged)} regions (no gap tolerance)")
        return merged
    ...
    def _refine_boundaries_with_vad(self, audio_np: np.ndarray, region: Tuple[float, float, str, float]) -> Optional[Tuple[float, float, str, float]]:
        ...
        Similar to the evaluation script's approach
        """
    ...
    def _save_report(self, regions: List[Tuple[float, float, str, float]], output_path: str, method: str):
        ...
        report_data = {
            "version": "frame_level_censor_v2",
            "merging_strategy": "any_profanity_no_gap",
        ...
        print(f"📋 V2 Censoring report saved to: {report_path}")
        
        # Print summary
        print("\n📊 V2 CENSORING SUMMARY:")
        print(f"   Version: frame_level_censor_v2")
        print(f"   Merging: any profanity, no-gap tolerance")
...
def main():
    ...
    print("🚀 Initializing Advanced Frame-Level Censoring System V2")
    print("🔄 V2 Features: Any profanity merging, no gap tolerance")
    print("=" * 70)
    
    censor = AdvancedFrameLevelCensorV2(
    ...
    print("\n✅ V2 Censoring completed successfully!")
