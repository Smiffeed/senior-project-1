...
class Wav2Vec2ForSpeechClassification(Wav2Vec2PreTrainedModel):
    ...
    def forward(
        ...
        if self.pooling_mode == "mean":
            ...
            pooled_output = torch.min(hidden_states, dim=1)[0]
...
def train_wav2vec2_model(csv_file, model_name, output_dir, pooling_mode='mean'):
    # Load dataset
    ...
    training_args = TrainingArguments(
        ...
        per_device_eval_batch_size=4,   # Increased from 4
        gradient_accumulation_steps=2,    # Reduced from 2 since we increased batch size
        save_strategy="steps",
...
if __name__ == "__main__":
    csv_file = './csv/mix_dataset.csv'  # Updated to use balanced training data
    model_name = "airesearch/wav2vec2-large-xlsr-53-th"
    
    mode = 'max'
    print(f"--- Training with {mode.upper()} pooling ---")
    output_dir = f'./models/audio_train_{mode}_pooling'
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(dataset)):
        ...
        train_wav2vec2_model(csv_file, model_name, output_dir_fold, pooling_mode=mean)
