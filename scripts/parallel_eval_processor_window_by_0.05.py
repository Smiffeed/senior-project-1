#!/usr/bin/env python3
"""
Parallel eval_percent batch processor - runs multiple evaluations simultaneously using evaluate_advanced_models_seperate.py (Method 2)
"""
...
def process_single_configuration_worker(config, model_path, threshold, output_dir, processor_script, worker_id):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"Worker {worker_id}: Output: evaluation_results/eval_by_0.05/{config['window']}/{config['stride']}/")
...
def main():
    ...
    parser.add_argument("--processor_script", default="scripts/simple_eval_processor_window.py", help="Processor script to use")
    ...
    print(f"Parallel eval_by_0.05 batch processor")
    print(f"Workers: {args.workers} parallel processes")
    print(f"CPU Count: {cpu_count}")
    print(f"Eval By 0.05 Dir: {args.eval_percent_dir}")
