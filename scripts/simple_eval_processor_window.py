#!/usr/bin/env python3
"""
Simple eval_percent processor with clean output filtering - v4 (Window-based)
Compatible with evaluate_advanced_models_window.py
"""
...
def get_plots_directory():
    """Get the plots directory, using worker-specific directory if available"""
    worker_plots_dir = os.environ.get('WORKER_PLOTS_DIR', './plots')
    return worker_plots_dir
...
def filter_evaluation_output(raw_output):
    ...
    keep_sections = [
        ...
        "Word-Level Recall:",
        "Word-Level F1-Score:",
        "=== IoU-based Word-Level Evaluation",
        "IoU Threshold:",
        "Total Ground Truth Words:",
        "Correct Predictions (IoU",
        "IoU-based Precision:",
        "IoU-based Recall:",
        "IoU-based F1-Score:",
        "Mean IoU:",
        ...
        "IoU vs Original GT",
        "✅ IoU vs original GT results saved",
        "🎯 ACCURACY COMPARISON:",
...
def process_single_file(csv_file, model_path, threshold, output_base_dir):
    ...
    plots_dir = Path(get_plots_directory())
    ...
    # Run evaluation with evaluate_advanced_models_window.py (window-based script)
    python_exe = "C:/Users/muldi/Documents/Playground/University/senior-project-1/env/Scripts/python.exe"
    cmd = [
        python_exe,
        "scripts/evaluate_advanced_models_window.py",
        "--csv_file", str(csv_file),
        "--model_path", str(model_path),
        "--threshold", str(threshold)
    ]
    
    print(f"Running evaluation and capturing clean output...")
    print(f"Clean output will be saved to: {note_file}")
    
    # Write header to note file
    header = f"""Window-Based Evaluation Results (v4)
=====================================
CSV File: {csv_file}
Model Path: {model_path}
Threshold: {threshold}
Output Directory: {output_dir}
Evaluation Method: Window-based (evaluate_advanced_models_window.py)
...
"""
    ...
    try:
        ...
        print("Running window-based evaluation (this may take a while)...")
        
        # Set environment variable for plots directory
        env = os.environ.copy()
        env['WORKER_PLOTS_DIR'] = str(plots_dir)
        ...
        if result.returncode == 0:
            print(f"Window-based evaluation completed successfully")
            ...
            with open(note_file, 'w', encoding='utf-8') as f:
                ...
                f.write(f"\n\n=== Window-based evaluation completed successfully ===\n")
                f.write(f"Completed at: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
                
                # Move plot files - simplified approach from v3
                moved_files = []
                if plots_dir.exists():
                ...
                results_csv = output_dir / "eval_results_advanced.csv"
                if results_csv.exists():
                    f.write(f"Results CSV: eval_results_advanced.csv\n")
                    print(f"Found results CSV: {results_csv}")
                
                print(f"Clean evaluation output saved to: {note_file}")
                print(f"Moved {len(moved_files)} plot files")
                return True
        else:
            print(f"Window-based evaluation failed with code: {result.returncode}")
            with open(note_file, 'w', encoding='utf-8') as f:
                f.write(header)
                f.write(f"=== Window-based evaluation failed with return code: {result.returncode} ===\n")
                f.write(result.stdout)
            return False
            
    except subprocess.CalledProcessError as e:
        print(f"Window-based evaluation failed: {e}")
        with open(note_file, 'w', encoding='utf-8') as f:
            f.write(header)
            f.write(f"=== ERROR ===\n{str(e)}\n")
        return False
    except KeyboardInterrupt:
        print(f"Window-based evaluation interrupted by user")
        with open(note_file, 'w', encoding='utf-8') as f:
            f.write(header)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Simple eval_percent processor with clean output (v4 - Advanced Models)")
    ...
    print(f"Simple eval_percent processor v4 (Window-based - clean output)")
    print(f"File: {args.csv_file}")
    print(f"Model: {args.model_path}")
    print(f"Threshold: {args.threshold}")
    print(f"Output: {args.output_dir}")
    print(f"Using: scripts/evaluate_advanced_models_window.py")
    ...
    if success:
        print(f"Window-based processing completed successfully!")
    else:
        print(f"Window-based processing failed!")
        sys.exit(1)
