#!/usr/bin/env python3
"""
🎯 4-STEP VAD EVALUATION USING EXISTING PREDICTIONS
Efficient evaluation that uses pre-computed results from fixed_smart_parallel_results
instead of re-running models

🔄 EVALUATION PROCESS:
Step 1: Load existing window/word predictions from fixed_smart_parallel_results
Step 2: Merge overlapping predictions into coherent regions  
Step 3: Apply VAD for temporal precision refinement
Step 4: IoU threshold analysis (0.1-0.9) with comprehensive metrics

🎨 ADVANTAGES:
- No model loading required - uses existing predictions
- Much faster processing - skip inference step
- Uses proven results from comprehensive evaluation
- Same 4-step methodology with VAD refinement
"""
...
def load_existing_predictions(results_dir: str, eval_type: str, evaluation_method: str, 
    ...
    Args:
        results_dir: Base results directory (fixed_smart_parallel_results)
        eval_type: 'eval_by_0.05' or 'eval_percent'  
        evaluation_method: 'window_eval', 'word_eval', 'iou_eval', 'word_iou_eval'
        window_size: Window size in seconds
        stride_value: Stride value
        stride_type: 'absolute' or 'percentage'
        
    Returns:
        DataFrame with existing predictions or None if not found
    """
    
    # Construct path to existing results
    if stride_type == 'absolute':
        stride_str = f"stride_{stride_value}s"
    else:  # percentage
        stride_str = f"stride_{stride_value}%"
    
    # Path structure: fixed_smart_parallel_results/eval_type/eval_type/method/window_Xs/stride_Y/
    ...
    possible_files = [
        'merged_predictions.csv',  # Primary file from fixed_smart_parallel_results
...
def parse_existing_predictions(predictions_df: pd.DataFrame) -> List[Tuple[str, List[Tuple[float, float, str, float]]]]:
    """
    Parse existing predictions into format needed for VAD processing
    
    Returns:
        List of (audio_file, [(start, end, label, confidence), ...])
    """
...
def merge_overlapping_windows(windows: List[Tuple[float, float, str, float]], max_gap: float = 0.3) -> List[Tuple[float, float, str, float]]:
    ...
    for next_start, next_end, next_label, next_conf in windows[1:]:
        # Merge if overlapping or gap is small and same label
        if next_start <= current_end + max_gap and next_label == current_label:
            current_end = max(current_end, next_end)
            current_conf = max(current_conf, next_conf)  # Keep higher confidence
        else:
...
def refine_boundaries_with_vad(audio_np: np.ndarray, region: Tuple[float, float, str, float], 
                             sample_rate: int = 16000) -> Tuple[float, float, str, float]:
    """
    Step 3: Apply VAD for temporal precision refinement
    """
    ...
    # Handle edge cases
    if start_sample >= len(audio_np) or end_sample <= start_sample:
        return region
    ...
    try:
        ...
        print(f"⚠️  VAD refinement failed for {label} at {start_time:.2f}s: {e}")
        return region
...
def process_vad_evaluation_from_existing(results_dir: str, eval_type: str, evaluation_method: str,
                                       window_size: float, stride_value: float, stride_type: str,
                                       ground_truth_file: str, output_dir: str):
    """
    Main function: 4-step VAD evaluation using existing predictions
    """
    
    print(f"🚀 4-Step VAD Evaluation Using Existing Predictions")
    print("=" * 60)
    ...
    # Step 1-2: Load existing predictions
    ...
    for i, (audio_file, raw_predictions) in enumerate(file_predictions, 1):
        ...
        # Step 2: Merge overlapping predictions
        merged_predictions = merge_overlapping_windows(raw_predictions)
        
        # Step 3: Apply VAD refinement
        try:
            # Load audio for VAD
            ...
            # Apply VAD refinement to each merged prediction
            refined_predictions = []
            for prediction in merged_predictions:
                refined = refine_boundaries_with_vad(audio_np, prediction)
                refined_predictions.append(refined)
            
        except Exception as e:
            print(f"  ⚠️  Warning: Could not load audio {audio_file} for VAD: {e}")
            refined_predictions = merged_predictions  # Use merged without VAD
        
        # Step 4: Calculate IoU for each ground truth word
        for gt_word in gt_data:
            ...
            for pred_start, pred_end, pred_label, pred_conf in refined_predictions:
    ...
    # Step 4: IoU threshold analysis and metrics calculation
    ...
    with open(output_path / 'note.txt', 'w', encoding='utf-8') as f:
        f.write("=== 4-STEP VAD EVALUATION USING EXISTING PREDICTIONS ===\n")
        ...
        f.write("Step 3: VAD temporal precision refinement\n")
    ...
    # Output summary
    print(f"\n✅ 4-STEP VAD EVALUATION COMPLETED (Using Existing Predictions):")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="4-Step VAD Evaluation Using Existing Predictions")
