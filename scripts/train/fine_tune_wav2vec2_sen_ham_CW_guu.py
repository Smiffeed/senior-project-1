...
# Define label mapping for binary classification (กู vs none)
label_map = {
    'none': 0,
    'กู': 1,
}

# Define the number of labels (binary classification)
...
def compute_metrics(eval_pred):
    ...
    return {
        ...
        # Add per-class F1 scores for binary classification
        "f1_none": f1[0] if len(f1) > 0 else 0.0,
        "f1_กู": f1[1] if len(f1) > 1 else 0.0,
    }
...
def augment_short_words(audio, label):
    """
    Enhanced augmentation specifically for the กู word in binary classification
    """
    if label == 'กู':  # Only augment the positive class in binary classification
        augmented = []
...
if __name__ == "__main__":
    csv_file = './csv/train_guu.csv'  # Use the binary classification dataset (กู vs none)
    model_name = "airesearch/wav2vec2-large-xlsr-53-th"
    output_dir = './models/binary_guu_vs_none'  # Updated to reflect binary classification
    
    print("🎯 BINARY CLASSIFICATION TRAINING: กู vs none")
